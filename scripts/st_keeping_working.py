...
class StationKeeping(Node):
    ...
    def imu_callback(self, msg):
        ...
        try:
            ...
            # self.get_logger().info(f"WAMV Euler Angles: Roll={self.roll}, Pitch={self.pitch}, Yaw={self.yaw}")
...
#     #     # Transform goal pose from GPS to ENU frame of reference
        ...
        # self.get_logger().info(f"GOAL Euler Angles: Roll={self.cmd_rot[2]}, Pitch={self.cmd_rot[1]}, Yaw={self.cmd_rot[0]}")
        ...
        if msg.data>0.5:
            ...
            if numpy.linalg.norm(err_pos) > self.goal_tol:
                ...
                if V_x > self.v_limit: # Clamp linear velocity along X-axis
                    V_x = self.v_limit
                V_y = 0.0
                err_rot = self.cmd_rot[0] - self.yaw # Error in orientation
                W_z = self.pid_g2g.control(err_rot, self.time) # PID controller for Wz
                W_z = math.atan(W_z)
                self.get_logger().info(f"AMAN:{self.cmd_rot[0] - self.yaw} OTHER: {W_z}")

                # self.get_logger().info(f"WELZ: {err_rot}")
                ...
                W_z = math.atan(W_z)
                self.get_logger().info(f"AMAN:{self.cmd_rot[0] - self.yaw} WZ val {W_z}")

                # self.get_logger().info(f"WELZ: {err_rot}")
 # PID controller for Wz
            ...
            '''def convert_velocity(x_vel, y_vel, orientation):
                # Linear velocity components
                linear_x = x_vel
                linear_y = y_vel
                linear_z = 0.0  # In the z-direction

                # Angular velocity component
                angular_x = 0.0
                angular_y = 0.0
                angular_z = orientation  # Assuming orientation is in radians

                # Create a Twist message to store the converted velocities
                twist_msg = Twist()
            ...
            twist_msg = convert_velocity(V_x, V_y, W_z)
            self.get_logger().info(f"twist msg {twist_msg}")'''
